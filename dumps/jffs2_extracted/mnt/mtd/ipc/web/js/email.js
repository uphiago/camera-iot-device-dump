...
/*	if (f.value == "")
	{
		alert(str_note_inputservname);
		f.select();
		return false;
	}
	if (checkHankakuNoKana(f.value, f, str_err_smptserv + comma + str_err_invaildc) == false)
		return false;
...
function change_form_malogintype()
{
...
function check_form_malogintype()
{
	...
	if (checkHankakuNoKana(fu.value, fu, str_err_username + comma + str_err_invaildc) == false)
		return false;
	if (checkHankakuNoKana(fp.value, fp, str_err_password + comma + str_err_invaildc) == false)
		return false;
	...
	if (checkProhibitedCharacter2(fp, fp.value) == false)
		return false;*/
	if(checkProhibitedCharacter2(fu,fu.value)==false)return false;
	if(checkProhibitedCharacter2(fp,fp.value)==false)return false;
	return true;
}
...
{
	var f1 = document.getElementById("form_mato1");
...
function ValidEmail(itemc)     // 检查EMAIL的合法性
{
	if (itemc.value.length > 0)
	{
		...
		{
			alert(str_err_emailaddr + comma + str_err_tooshort + exclamation);
			itemc.select();
			return false;
		}
		...
		{
			alert(str_err_emailaddr + comma + str_err_noat + exclamation);
			itemc.select();
			return false;
		}
	}
...
function check_form_mato()
{
	...
	/*if ((f1.value == ""))
	{
		alert(str_note_inputemail);
		f1.select();
		return false;
	}

	if (checkHankakuNoKana(f1.value, f1, str_err_addr1 + comma + str_err_invaildc) == false)
		return false;

	if (checkProhibitedCharacter2(f1, f1.value) == false)
		return false;

	if (ValidEmail(f1) == false)
		return false;*/
		if(checkProhibitedCharacter2(f1,f1.value)==false)return false;
...
/*	if (f.value == "")
	{
		alert(str_note_inputsendaddr);
		f.select();
		return false;
	}
	
	if (checkHankakuNoKana(f.value, f, str_err_sendaddr + comma + str_err_invaildc) == false)
		return false;
	if (checkProhibitedCharacter2(f, f.value) == false)
		return false;
	if (ValidEmail(f) == false)
		return false;*/
	if(checkProhibitedCharacter2(f,f.value)==false)return false;
...
function check_form_masubject()
{
	...
	/*if (checkHankakuNoKana(f.value, f, str_err_subject + comma + str_err_invaildc) == false)
		return false;
	if (checkProhibitedCharacter2(f, f.value) == false)
		return false;*/
		if(checkProhibitedCharacter2(f,f.value)==false)return false;
